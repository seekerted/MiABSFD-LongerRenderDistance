-- Ted the Seeker's utils.lua 2024-03-22
local Utils = {}
...
-- Register a hook that provides an Unregister function (as the first argument) for easy unregistering
function Utils.RegisterEasyUnregisterHook(FunctionName, Function)
	local PreID = nil
	local PostID = nil
	...
	local function Unregister()
		...
		Utils.Log("Unhooked (easy unregister) %s (%d, %d)", FunctionName, PreID, PostID)
	end
	...
	Utils.Log("Hooked (easy unregister) onto %s (%d, %d)", FunctionName, PreID, PostID)
end
